...
export class ServicioService {
  public resourceUrl = this.applicationConfigService.getEndpointFor('api/servicios');
  ...
  update(servicio: IServicio): Observable<EntityResponseType> {
    return this.http.put<IServicio>(`${this.resourceUrl}/${getServicioIdentifier(servicio) as number}`, servicio, { observe: 'response' });
  }

  partialUpdate(servicio: IServicio): Observable<EntityResponseType> {
    return this.http.patch<IServicio>(`${this.resourceUrl}/${getServicioIdentifier(servicio) as number}`, servicio, {
      observe: 'response',
    });
  }
